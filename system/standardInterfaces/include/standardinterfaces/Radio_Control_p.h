...
namespace RX_Control {
  ...
  public:
    ...
    /// Set the number of active channels
    void set_number_of_channels(CORBA::ULong num){ base->set_number_of_channels(num); };
    void get_number_of_channels(CORBA::ULong& num){ base->get_number_of_channels(num); };

    /// Gain functions
    void get_gain_range(CORBA::ULong channel, float& gmin, float& gmax, float& gstep) { base->get_gain_range(channel, gmin, gmax, gstep); };
    void set_gain(CORBA::ULong channel, float gain) { base->set_gain(channel, gain); };
    void get_gain(CORBA::ULong channel, float& gain) { base->get_gain(channel, gain); };

    /// Frequency tuning functions
    void get_frequency_range(CORBA::ULong channel, float& fmin, float& fmax, float& fstep) { base->get_frequency_range(channel, fmin, fmax, fstep); };
    void set_frequency(CORBA::ULong channel, float f) { base->set_frequency(channel, f); };
    void get_frequency(CORBA::ULong channel, float& f) { base->get_frequency(channel, f); };

    // Start/Stop Control
    void start(CORBA::ULong channel) { base->start(channel); };
    void stop(CORBA::ULong channel) { base->stop(channel); }; 

    // Set properties
    void set_values(const CF::Properties& values) { base->set_values(values); };

   /// Set the receiver decimation rate
    void set_decimation_rate(CORBA::ULong channel, CORBA::ULong M) { base->set_decimation_rate(channel, M); };
    void get_decimation_range(CORBA::ULong channel, CORBA::ULong& dmin,
CORBA::ULong& dmax, CORBA::ULong& dstep) { base->get_decimation_range(channel, dmin, dmax, dstep); };

    /// Set data output size
    void set_data_packet_size(CORBA::ULong channel, CORBA::ULong N) { base->set_data_packet_size(channel, N); };
...
namespace TX_Control {
  ...
  public:
   ...
   /// Set the receiver decimation rate
    void set_interpolation_rate(CORBA::ULong channel, CORBA::ULong M) { base->set_interpolation_rate(channel, M); };
    void get_interpolation_range(CORBA::ULong channel, CORBA::ULong& dmin,
CORBA::ULong& dmax, CORBA::ULong& dstep) { base->get_interpolation_range(channel, dmin, dmax, dstep); };
