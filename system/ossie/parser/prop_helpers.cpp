...
// by RADMOR
CORBA::ULongLong ossieSupport::strings_to_unsigned_long_long(std::vector<std::string> &values)
{
    CORBA::ULongLong result(0);

    result = (unsigned long long) strtoull(values[0].c_str(), NULL, 10);
...
// by RADMOR
CORBA::ULongLongSeq *ossieSupport::strings_to_unsigned_long_long_sequence(std::vector<std::string> &values)
{
    ...
    for (unsigned int i = 0; i < values.size(); ++i) {
	result[i] = (unsigned long long) strtoull(values[i].c_str(), NULL, 10);
    }
